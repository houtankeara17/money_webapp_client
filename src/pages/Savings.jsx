...
const Savings = () => {
  ...
  useEffect(() => {
    fetchData(year);
  }, [year, filterCategory, filterMonth]);
  ...
  const doDelete = async () => {
    ...
    try {
      if (confirmDel === "all") {
        ...
        await api.delete(`/savings/${confirmDel}`);
  ...
  const handleExport = async () => {
    try {
      const { data } = await api.get("/savings/export");
      ...
      a.download = `savings-${Date.now()}.json`;
      a.click();
  ...
  return (
    <div className="w-full min-h-full">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            ...
            {CATEGORIES.map((c) => (
              <button
                key={c}
                type="button"
                onClick={() => setFilterCategory((cur) => (cur === c ? "" : c))}
                className={`px-2.5 py-1 rounded-full text-[11px] font-medium transition ${
                  filterCategory === c
                    ? "bg-teal-600 text-white"
                    : "bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700"
      ...
      >
        ...
        <Modal.Body>
          <form id="saving-form" onSubmit={handleSubmit} className="space-y-4">
              ...
              <tbody>
                {pagedItems.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-3.5">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${CATEGORY_TONE[item.category] || CATEGORY_TONE.Other}`}
                      >
                        {CATEGORY_EMOJI[item.category]} {tEnum(item.category)}
                      </span>
                    </td>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {pagedItems.map((item) => (
              ...
              >
                <div className="mb-3">
                  <div
                    className={`w-full h-28 rounded-xl flex flex-col items-center justify-center gap-1.5 ${CATEGORY_TONE[item.category] || CATEGORY_TONE.Other}`}
                  >
            ...
            {pagedItems.map((item) => (
              ...
              >
                <div className="min-w-0 flex items-center gap-3.5">
                  <div
                    className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 text-xl ${CATEGORY_TONE[item.category] || CATEGORY_TONE.Other}`}
                  >
                    {CATEGORY_EMOJI[item.category] || "💰"}
                  </div>
